...
[![Gravitee.io](https://img.shields.io/static/v1?label=Available%20at&message=Gravitee.io&color=1EC9D2)](https://download.gravitee.io/#graviteeio-apim/plugins/policies/gravitee-policy-generate-http-signature/)
...
[![Available at Gravitee.io](https://img.shields.io/static/v1?label=Available%20at&message=Gravitee.io&color=1EC9D2)](https://download.gravitee.io/#graviteeio-apim/plugins/policies/gravitee-policy-webhook-signature-generator/)
...
## Phases
The `generate-http-signature` policy can be applied to the following API types and flow phases.
...
|:----------------------|:-----------------------|:----------:|:---------|:-------------|
| Algorithm<br>`algorithm`| enum (string)| ✅| `HMAC_SHA256`| The HMAC digest algorithm.<br>Values: `HMAC_SHA1` `HMAC_SHA256` `HMAC_SHA384` `HMAC_SHA512`|
| Include the created timestamp in the signature<br>`created`| boolean|  | `true`| Add (created) header in the signature. (Highly recommended)|
| Include the expires timestamp in the signature<br>`expires`| boolean|  | `true`| Add (expires) header in the signature. (Highly recommended)|
| Headers<br>`headers`| array (string)|  | | List of headers to use for HTTP signature creation.|
| Delimiter<br>`headersDelimiter`| string|  | `.`| Specify a delimiter to separate each header and the body/message|
| Key ID<br>`keyId`| string| ✅| | The key ID used to sign (support EL).|
| Prepend headers to body for signature generation<br>`prependHeadersToBody`| boolean|  | | The defined headers will be prepended to the body for signature generation. (Only applicable if the signPaylonad option is enabled)|
| Scheme<br>`scheme`| enum (string)| ✅| `AUTHORIZATION`| Signature Scheme.<br>Values: `AUTHORIZATION` `SIGNATURE` `CUSTOM_HEADER`|
| Secret<br>`secret`| string| ✅| | The secret key used to sign (support EL).|
| Include headers in signature generation<br>`signHeaders`| boolean|  | `true`| Base signing process on the request/response/message headers|
| Include request method in signature generation<br>`signMethod`| boolean|  | `true`| Base signing process on the request method name (Highly recommended for request flow)|
| Include payload in signature generation<br>`signPayload`| boolean|  | | Base signing process on the request/response/message payload (Required for response flow)|
| Include request URI in signature generation<br>`signUri`| boolean|  | `true`| Base signing process on the request URI name (Highly recommended for request flow)|
| Custom Signature Header<br>`targetSignatureHeader`| string|  | `X-HMAC-Signature`| Specify the HTTP header that will contain the generated HMAC signature|
| Validity duration<br>`validityDuration`| integer<br>`[1, +Inf]`|  | `3`| Signature's maximum validation duration in seconds.|
...
*Proxy API on Response phase - Basic signature*
```json
{
  "api": {
    "definitionVersion": "V4",
    "type": "PROXY",
    "name": "Generate HTTP Signature example API",
    "flows": [
      {
        "name": "Common Flow",
        "enabled": true,
...
```
*Proxy API on Response phase - Signature with additional headers*
```json
{
  "api": {
    "definitionVersion": "V4",
...
```
*Response phase with secret from Secret Manager*
```json
{
  "api": {
    "definitionVersion": "V4",
